...
class WorkingSeeder extends Seeder
{
    ...
    {
        // 💡 管理者(admin=true)を除いた、実際に打刻データを持つべき一般社員のみを対象にする
        //    ※ ID固定(1)をやめて、DBに存在する全ての一般社員に対して生成するよう変更
        $userIds = User::where('admin', false)->pluck('id');

        if ($userIds->isEmpty()) {
            $this->command?->warn('打刻データの生成対象ユーザー（admin=false）が見つかりませんでした。UserSeederを先に実行してください。');
            return;
        }

        // 💡 再実行時に重複登録されないよう、対象ユーザーの既存データを一旦削除
        Working::whereIn('user_id', $userIds)->delete();
