...
class WorkingSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        for ($date = $startOfMonth->copy(); $date->lte($endOfMonth); $date->addDay()) {
            ...
            $randomMinuteOut = rand(0, 20);  // 18:00 〜 18:20

            Working::create([
                'user_id' => 1,
                'target_date' => $date->format('Y-m-d'), // シフトの定義に合わせtarget_dateと仮定
                'attendance' => $date->copy()->setTime(9, 0, 0)->addMinutes($randomMinuteIn)->format('H:i:s'),
                'leaving' => $date->copy()->setTime(18, 0, 0)->addMinutes($randomMinuteOut)->format('H:i:s'),
                'working_place' => null,  
                'commute' => 350,
                'status' => '承認',
                'created_at' => now(),
